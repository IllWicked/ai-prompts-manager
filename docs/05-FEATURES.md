...
| Функция | Описание |
|---------|----------|
| `detectLanguageInText(text)` | Находит язык по любой форме слова |
| `detectAllLanguagesInText(text)` | Все языки в тексте |
...
```javascript
// Автоматически определяет падеж и подставляет правильную форму
replaceLanguage('для англоязычной аудитории в английском сегменте', 'en', 'de');
// → 'для немецкоязычной аудитории в немецком сегменте'

// Сохраняет регистр первой буквы
replaceLanguage('Английские пользователи', 'en', 'es');
// → 'Испанские пользователи'

// Замена кода локали
replaceLanguage('Локаль: en-GB', 'en', 'de');
// → 'Локаль: de-DE'
```
...
| Код | Язык | Мультигео |
|-----|------|-----------|
| en | английский | 🌍 5 стран (US, GB, CA, AU, NZ) |
| de | немецкий | 🌍 3 страны (DE, AT, CH) |
