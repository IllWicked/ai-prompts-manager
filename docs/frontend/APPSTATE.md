...
window.AppState = {
    workflow: {
        ...
        colors: {},              // Цвета блоков {blockId: "#hex"}
        zoom: 0.6                // Текущий zoom (0.2-1.25)
    },
    interaction: {
...
36 алиасов для упрощения доступа к состоянию:
...
workflowColors ↔ AppState.workflow.colors
...
| Dynamic Input values | `field-value-{tabId}-{blockId}-{index}` | dynamic-input.js |
...
- **SNAPSHOT_DEBOUNCE_MS = 300** — набор текста группируется в один snapshot
