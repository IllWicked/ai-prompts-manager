...
Справочник функций приложения. Общее количество ~290 функций в 35 модулях (включая внутренние). В таблице ниже — **публичные API функции** (~96), сгруппированные по категориям.
...
| [Language System](#language-system) | 11 | `language-ui.js` | Детекция и замена языков |
...
| [Edit Toolbar](#edit-toolbar) | 3 | `edit-helpers.js` | Тулбар редактирования |
...
| [Data Persistence](#data-persistence) | 8 | `persistence.js` | Хранение данных |
...
```javascript
// Замена с автоопределением падежа
replaceLanguage('для англоязычной аудитории', 'en', 'de');
// → 'для немецкоязычной аудитории'

// Замена кодов локали
replaceLanguage('Локаль: en-GB, язык: en', 'en', 'de');
// → 'Локаль: de-DE, язык: de'
...
| `showPromptTools()` | Показать инструменты промпта |
...
| Функция | Описание |
|---------|----------|
| `convertToTabStructure(id, name, order, blocks)` | Конвертация в структуру вкладки |
