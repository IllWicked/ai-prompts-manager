...
Промпты используют маркеры `{{lang:nom.m}}`, `{{native:gen.f}}`, `{{country}}`, `{{locale}}` вместо хардкод-значений. Язык управляется только через селектор (сохраняется в localStorage).

### Замена языков

При смене языка в селекторе:
- `applyLanguage()` / `applyLanguageWithCountry()` обновляют `currentLanguage` + `currentCountry`
- Вызывается `renderWorkflow()` — маркеры раскрываются в новые значения
- Данные блоков не меняются (маркеры остаются)
