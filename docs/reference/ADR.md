...
Файл загружается в Rust через `include_str!()` и передаётся в WebView через `window._s`.
...
function __getSel__(path) {
    return path.split('.').reduce((obj, key) => obj?.[key], window._s);
}
...
После роста кодовой базы (34 JS модулей, ~372 функций) ручное тестирование стало недостаточным для отлова регрессий в критичных модулях.
...
Разбить монолитный `main.rs` на **22 файла** по функциональности:

```
src-tauri/src/
├── main.rs        (140 строк) — только setup
├── lib.rs         — реэкспорт
├── types.rs       — структуры данных
├── state.rs       — глобальные состояния
├── commands/      — 9 файлов с 51 командой
...
```markdown
## ADR-011: WebView Coordination — SetWindowPos + offscreen positioning
...
2. **Offscreen positioning** — `set_position(width*2, 0)` для неактивных табов. IsVisible=TRUE, DOM живой. `hide()` отключён — `put_IsVisible(FALSE)` замораживал timers.

3. **Suspend отключён** — `TrySuspend()` замораживал DOM полностью (querySelector видел замороженный стейт). Offscreen + IsVisible=TRUE достаточно для снижения нагрузки.
...
- Добавлены: `raise_toolbar_zorder()`, `layout_ui()`, `layout_claude()`, `layout_overlay()`. `suspend_claude_tab()`/`resume_claude_tab()` — no-op
- Зависимости: `tokio = { features = ["time"] }`, `windows = { features = ["Win32_UI_WindowsAndMessaging", "Win32_Foundation"] }`
- Платформо-зависимые функции (SetWindowPos) — no-op на не-Windows. Suspend/Resume отключены

---

## ADR-012: Claude Counter без monkey-patching
...
Нужно интегрировать плагин Claude Counter (usage bars, cache timer) в Claude WebView. Оригинальный плагин перехватывает `window.fetch` и `history.pushState/replaceState` — monkey-patching, который был удалён в v4.3.4 как вероятная причина блокировок аккаунтов.

### Решение

Переписать плагин без monkey-patching. Убрать bridge pattern (не нужен в WebView2 — единый контекст). Убрать токенизатор o200k_base (~2MB) — подсчёт токенов бесполезен после компактизации контекста.

### Причины

| Оригинал | Замена |
|----------|--------|
| `window.fetch` wrapper | Прямой `fetch()` к API эндпоинтам |
| SSE event-stream перехват | Refresh при окончании генерации (URL hash) |
| `history.pushState` patch | `popstate` + polling 1.5 сек |
| Bridge postMessage | Не нужен — единый контекст |

### Последствия

- Usage обновляется не мгновенно (при окончании генерации + hourly + rollover), а не в реальном времени из SSE. Приемлемо.
- Token count не отображается. Бесполезен после компактизации, а generic токенизатор неточен относительно Claude.

---

## ADR-013: Knowledge Upload — мгновенная загрузка вместо буфера
...
При работе с пайплайном Claude генерирует промежуточные MD-файлы, которые нужны в knowledge проекта для последующих чатов. Рассматривались два подхода.

### Альтернативы

1. **Буфер** — скачанные MD копятся, загружаются в knowledge при New Chat (флаг N)
2. **Мгновенная загрузка** — каждый скачанный MD сразу уходит в knowledge

### Решение

Мгновенная загрузка (вариант 2).

### Причины

| Фактор | Буфер | Мгновенная |
|--------|-------|------------|
| Состояние | Нужно хранить, персистить, очищать | Ноль состояния |
| Доступность | Только после New Chat | Сразу во всех чатах |
| Привязка | К флагу N | К любому скачиванию |
| Ошибки | Файл застрянет если нет New Chat | Ошибка видна сразу (toast) |

### Последствия

- Файл удаляется с диска после загрузки (юзеру не нужен)
- Загрузка только при активном проекте (`isProjectActive()`)
- Фильтр только по расширению `.md`
