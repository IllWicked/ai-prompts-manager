...
## [4.4.21] - 2026-04-28 {#v4421}

### Новое

- **Турецкий язык + Турция:** добавлен в выпадающий список языков. Одногеошный (без LANGUAGE_COUNTRIES). Локаль `tr-TR`. Склонения «турецкий» и «турецкоязычный» генерируются автоматически из nominative через `generateAdjectiveForms()`, спецкейсов не потребовалось (туре_цк_ → isSkij ветка, турецкоязычн_ → soft ветка). Privacy/about/legal/cookie переведены на турецкий.

### Изменённые файлы

| Файл | Изменения |
|------|-----------|
| `dist/index.html` | Добавлена `.language-option` для `tr` после `sl` |
| `dist/js/language-ui.js` | Добавлен `'tr': 'TR Турецкий'` в `langTexts` после `sl` |
| `dist/js/languages.js` | Добавлена `tr` запись в `LANGUAGES` (lang, native, country, locale, 4 правовых поля) |
