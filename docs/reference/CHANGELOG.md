...
- **Решение:** Сетка перенесена на `::before` псевдоэлементы, анимация заменена на `transform: translateY()` (GPU-композитное — ноль перерисовок). Glow-дубли удалены, свечение в тёмной теме реализовано через статичный `drop-shadow`. Количество анимированных DOM-элементов уменьшено с 4 до 2, ширина плоскостей с 200% до 150%
