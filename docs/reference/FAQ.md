...
Настройки → секция «Диагностика» → кнопка «Экспорт». Файл `diagnostics.json` сохраняется на рабочий стол. Содержит историю health-check селекторов, эвристических fallback и других диагностических событий.
...
Исторически данные хранились только в localStorage ([ADR-002](ADR.md#adr-002-localstorage-вместо-indexeddb)). С v4.2.5 используется **гибридное хранение**: localStorage (быстрый синхронный кэш) + файл `tabs_data.json` в App Data Dir (резервная копия). При повреждении localStorage данные автоматически восстанавливаются из файла.
